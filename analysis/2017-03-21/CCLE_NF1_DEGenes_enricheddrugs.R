...
degenes<-read.table(synGet("syn8506646")@filePath, header = TRUE, sep = ",")
gene<-rownames(degenes)
degenes<-cbind(gene, degenes)
degenes<-filter(degenes, adj.P.Val<=0.1 & logFC<=0)
degenes<-arrange(degenes, logFC)
...
testgenes<-unique(degenes$gene)

hyper<-TestForDrugTargets(testgenes)

write.table(hyper, "CCLE_mutation_enriched_drugs.txt", sep = "\t")
synStore(File("CCLE_mutation_enriched_drugs.txt", parentId = "syn8494777"), used = c("syn7341038", "syn8118065"), executed = this.file)
